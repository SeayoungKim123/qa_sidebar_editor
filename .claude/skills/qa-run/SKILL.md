...
## 메일 AI 사이드패널 시나리오 진입 절차 (SCN-01~05, 현행)

현재 활성 시나리오는 메일 5종(`scenarios/01-메일-백지작성.md` … `05-메일-추천기능칩.md`)이다. 이들은 드라이브·OnlyOffice 가 아니라 **메일 쓰기 페이지의 우측 AI 어시스턴트 패널**을 검증한다. 아래 진입 절차를 **순서대로** 따른다.
