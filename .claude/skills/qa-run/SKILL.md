...
1. 로그인(`login.devoffice.hiworks.com` 2단계) → 메일 쓰기 `mails.devoffice.hiworks.com/write?mode=normal` 직접 진입. "보안을 위하여 다시 로그인" 다이얼로그가 뜨면 수락 후 재로그인. "인앱마케팅 모달"·"AI채팅 Pro" 다이얼로그는 닫기.
2. **① 메일 LNB 축소 (필수)**: 메일 LNB 우측 끝 드래그 핸들(`.DragResizable_drag__*`, x≈276)을 **더블클릭** → 아이콘 레일로 축소. 이 단계를 빼면 본문이 좁아 캡처·검증이 흐트러진다. (ref 가 안 잡히면 `browser_click` 의 `target` 에 셀렉터 `.DragResizable_drag__QVffO` 를 주고 `doubleClick:true`.)
3. 우측 상단 **보라색 "AI 채팅 패널 토글"** 클릭 → 우측 iframe 패널(`#ai-chat-iframe-panel`, 폭 400px) 노출. "작성 중인 메일 참조 중" 배지·추천기능 칩 확인.
4. **② AI 채팅 내부 `사이드바 토글`은 누르지 않는다**: 메일 패널은 400px 고정이라 내부 사이드바가 **항상 모바일 모드**다. 기본 상태에서 채팅기록 드로어는 **이미 닫혀 있고 채팅 영역(추천기능·입력창)이 보인다** → 토글을 누를 이유가 없다. (자세한 함정은 아래 "③ 에디터 조작 실무"의 LNB 항목 참조.)
5. 모델 기본값은 **Claude Sonnet 4.6**(패널 하단 모델 선택 버튼으로 확인). 전송은 입력창 포커스 후 **Enter**. 토큰은 응답 말미 `Tokens / input: N | output: M | total: T` 줄에서 읽는다.
