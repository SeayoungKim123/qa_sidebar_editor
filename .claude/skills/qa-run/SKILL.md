...
- **AI 채팅 입력 → 전송**: 입력창(`f2e*` textbox)에 `browser_type` 으로 프롬프트 입력 → `02-prompt` 캡처 → **`t_send = browser_evaluate(() => Date.now())` (필수, 빼먹으면 wait_ms 가 null) → 곧바로 `전송` 버튼 클릭**. 모델은 시나리오 지정값(현재 `GPT 5.4`)인지 패널 하단 모델 선택 버튼으로 확인. "AI채팅 Pro로 업그레이드 되었습니다" 다이얼로그는 닫기(X).
