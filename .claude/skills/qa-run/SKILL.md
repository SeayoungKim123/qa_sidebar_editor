...
## 메일 AI 사이드패널 시나리오 진입 절차 (SCN-01~06, 현행)

현재 활성 시나리오는 메일 6종(`scenarios/01-메일-백지작성.md` … `06-메일-추천기능칩.md`)이다. 이들은 드라이브·OnlyOffice 가 아니라 **메일 쓰기 페이지의 우측 AI 어시스턴트 패널**을 검증한다. 아래 진입 절차를 **순서대로** 따른다.

> ⚠️ **화면엔 "LNB" 가 두 개다 — 혼동 금지.**
> - **① 메일 페이지 LNB** = 좌측 메일함 목록(전체/받은/보낸…). 폭 ~276px. **이걸 접는다(진입 2단계).**
> - **② AI 채팅 패널 내부 사이드바** = AI iframe 안의 `새 대화`·`검색`·`채팅기록` 목록. **이건 건드리지 않는다(아래 4단계).**
> 접어야 하는 건 ①뿐이고, 이는 **사이드패널 테스트에서 본문 캡처 공간을 확보하기 위한** 진입 단계다.

### 진입 사전점검 (PRE-FLIGHT) — 시나리오마다 필수 · 측정 기반 게이트

> 🚦 **세션이 새 것이든 직전 시나리오에서 이어받았든 동일하게 실행한다. "이전에 해놨으니 됐다"는 가정 금지.** 진입 셋업은 "최초 1회"가 아니라 **매 시나리오의 불변식(invariant)**이다. 첫 TC 의 어떤 조작·캡처보다 먼저, 아래 5개를 **순서대로** 통과시킨다. 3번은 눈짐작이 아니라 **숫자로 측정해 판정**한다(과거 세션 재사용 시 LNB 가 리로드로 되돌아가 있던 누락의 재발 방지).

1. **모달/알림 클리어**: "보안을 위하여 다시 로그인"(수락 후 재로그인) · "인앱마케팅 모달" · "AI채팅 Pro" · `서버 오류(500)` 등 다이얼로그가 있으면 모두 처리(수락/닫기). `browser_handle_dialog` 또는 닫기 버튼.
2. **위치 확인**: 현재 URL 이 `mails.devoffice.hiworks.com/write?mode=normal` 이 아니면 `browser_navigate` 로 이동.
3. **① 메일 LNB 접힘 — 측정 게이트(핵심, 가정 금지)**: 메일 LNB 는 부모 페이지(동일 출처)라 `browser_evaluate` 로 잴 수 있다. 드래그 핸들 위치를 측정해 펼쳐졌으면 접는다:
   ```js
   () => { const h = document.querySelector('[class*="DragResizable_drag"]');
           return h ? Math.round(h.getBoundingClientRect().left) : -1; }
   ```
   - 반환값 **> 150 → 펼쳐짐**: 드래그 핸들(`.DragResizable_drag__QVffO`) **더블클릭**(`browser_click` `doubleClick:true`) → **다시 측정**해 **≤ 100(아이콘 레일)** 확인. 안 접히면 재시도 1회.
   - 반환값이 이미 ≤ 100 이면 접힌 것 — 통과.
   - `-1`(핸들 못 찾음)이면 페이지가 덜 떴거나 구조 변경 — 짧게 대기 후 재측정, 그래도 -1 이면 메인 요약에 1줄 보고.
4. **AI 패널 열림 확인**: `#ai-chat-iframe-panel` 존재 확인. 없으면 우측 상단 **보라색 "AI 채팅 패널 토글"** 클릭. "작성 중인 메일 참조 중" 배지 확인. (**② AI 채팅 내부 `사이드바 토글`은 누르지 않는다** — 메일 패널 400px 모바일 드로어 모드라 누르면 채팅을 가린다. 자세한 함정은 아래 "③ 에디터 조작 실무"의 LNB 항목.)
5. **모델 = Claude Sonnet 4.6** 확인(패널 하단 모델 선택 버튼). 다르면 메인 요약에 보고.

→ **5개가 모두 통과해야 첫 TC 진행.** 하나라도 실패하면 메인 반환 요약에 한 줄로 보고한다. 전송은 입력창 포커스 후 **Enter**(전송 버튼 금지). 토큰은 응답 말미 `Tokens / input: N | output: M | total: T` 줄에서 읽는다.

> ⚠️ **화면엔 "LNB" 가 두 개다 — 혼동 금지.**
> - **① 메일 페이지 LNB** = 좌측 메일함 목록(전체/받은/보낸…). 펼침 폭 ~276px. **이걸 접는다(위 3단계, 측정 게이트).**
> - **② AI 채팅 패널 내부 사이드바** = AI iframe 안의 `새 대화`·`검색`·`채팅기록` 목록. **이건 건드리지 않는다(위 4단계).**

본문 직접 편집 금지·빈 문서 복원·전송 규칙은 아래 DOCX/PPT/Excel 규칙의 2)·3) 과 동일하게 적용한다(엔진만 메일 에디터로 대체).

---

## AI 시나리오 전용 규칙 (DOCX / PPT / Excel, 레거시)
...
- **AI 채팅 내부 사이드바(채팅기록) 처리 — 패널 폭에 따라 정반대다. 메일 패널에선 토글을 누르지 말 것**:
  ...
  - **좁은 패널(메일 패널 400px, 모바일 드로어 모드) — 현행 SCN-01~05**: 채팅기록은 **기본적으로 닫힌 드로어**라 채팅 영역이 이미 보인다. 여기서 `사이드바 토글`을 누르면 목록이 *접히는 게 아니라* **backdrop(`fixed inset-0 z-50`) 딸린 드로어가 펼쳐져** 채팅을 통째로 가린다. ⇒ **메일 패널에선 토글을 누르지 않는다.** 기본 상태 그대로 캡처.
